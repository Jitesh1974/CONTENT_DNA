...
"""
        ...
        style_section = f"""
Style guidelines:
- Average sentence length: {brand_dna.get('avg_sentence_length', 15)} words
...
Content:"""
    ...
    def _optimize_content(self, content: str, brand_dna: Dict, platform: str) -> str:
        ...
        for i, line in enumerate(lines):
            ...
            if platform == "blog" and i == 0 and not line.startswith('#'):
        ...
        if platform in ["linkedin", "instagram"] and '#' not in content:
            keywords = brand_dna.get("primary_keywords", [])[:4]
            hashtags = ' '.join([f"#{kw.replace(' ', '')}" for kw in keywords if kw])
            content = f"{content}\n\n{hashtags}"
        
        # Ensure CTA for marketing content
        if platform in ["linkedin", "email"] and not self._has_cta(content):
            ctas = {
    ...
    def _calculate_content_score(self, content: str, brand_dna: Dict, platform: str) -> int:
        ...
        target_words = {"linkedin": 250, "instagram": 125, "blog": 1000, "email": 200}
        ...
        score += int((keyword_matches / max(1, len(keywords))) * 10)
        
        # Structure score (10 points)
        if platform == "blog" and content.count('\n#') >= 2:
            score += 10
